...
import { apiCreateOrientation, apiSearchOrientation } from "../../api/axios";
...
interface ICreateOrientationFromInput {
  file: any;
}
...
export const CreateOrientation = () => {
  ...
  const { data: searchOrientationData } = useQuery<ISearchCoreOutput>(
    [
      "searchOrientation",
      "pagesize:" + pagesize,
      "page:" + page,
  ...
  const createOrientationMutation = useMutation(apiCreateOrientation, {
    onSuccess: (data: ICoreOutput) => {
      if (data.ok) {
        ...
        naviage(
          `/search-orientation?pagesize=${pagesize}&page=1&normid=${query.normid}`
        );
      }
    },
  });
  ...
  const {
    ...
    getValues,
    reset,
  } = useForm<ICreateOrientationFromInput>({
    mode: "onChange",
  });

  const onSubmit = () => {
    if (
      !createOrientationMutation.isLoading &&
      typeof searchOrientationData?.totalResult === "number" &&
      query.normid
    ) {
      const { file } = getValues();
      createOrientationMutation.mutate({
        page: searchOrientationData?.totalResult + 1,
        normId: +query.normid,
        file: file[0],
      });
    }
    ...
    <div className="mt-5 max-w-5xl mx-auto rounded-2xl  flex flex-col justify-center items-center p-10 font-bold">
      ...
      <div className="grid grid-cols-1 grid-rows-none  w-full  mx-auto grid-flow-col gap-20 ">
        <div className=" row-start-1 ">
          <h4 className="font-semibold text-2xl  text-center ">OT 등록</h4>
          <div className="flex justify-between items-center min-w-max">
            <div>
              <Link
                className="hover:underline "
                to={`/search-orientation?pagesize=${pagesize}&page=1&normid=${query.normid}`}
              >
              ...
              <span className="col-start-1 col-row-1 my-2">
                ({" "}
                {typeof searchOrientationData?.totalResult === "number" &&
                  searchOrientationData?.totalResult + 1}
                페이지 )
              </span>
            </div>
          </div>
