...
import { apiEditQuestion, apiSelectQuestion } from "../../api/axios";
...
interface IEditQuestionFromInput {
  file: any;
}
...
export const EditQuestion = () => {
  ...
  const { data: questionData } = useQuery(
    ["question", "normId:" + normId, "page:" + page],
    () => apiSelectQuestion({ normId, page })
  );

  const editQuestionMutation = useMutation(apiEditQuestion, {
    onSuccess: (data: ICoreOutput) => {
      if (data.ok) {
        ...
        naviage(
          `/search-question?pagesize=${pagesize}&page=1&normid=${
  ...
  const {
    ...
    getValues,
    reset,
  } = useForm<IEditQuestionFromInput>({
    mode: "onChange",
  });

  const onSubmit = () => {
    if (!editQuestionMutation.isLoading) {
    ...
    <div className="mt-5 max-w-5xl mx-auto rounded-2xl font-bold  flex flex-col justify-center items-center p-10 ">
      ...
      <div className="grid grid-cols-1 grid-rows-none  w-full  mx-auto grid-flow-col gap-20 ">
        <div className=" row-start-1 ">
          <h4 className="font-semibold text-2xl  text-center ">문제 수정</h4>
                ...
                {imageSrc && isValid ? (
                  ...
                  questionData && (
                    <div>
                      ...
                      <hr className=" w-full border-gray-300 mt-5" />
                      <img src={questionData.imageUrl} alt="" />
                    </div>
                  )
                )}
              </div>
