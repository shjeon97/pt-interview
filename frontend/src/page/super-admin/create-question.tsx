...
import { apiCreateQuestion, apiSearchQuestion } from "../../api/axios";
...
interface ICreateQuestionFromInput {
  file: any;
}
...
export const CreateQuestion = () => {
  ...
  const { data: searchQuestionData } = useQuery<ISearchCoreOutput>(
    [
      "searchQuestion",
      "pagesize:" + pagesize,
      "page:" + page,
  ...
  const createQuestionMutation = useMutation(apiCreateQuestion, {
    onSuccess: (data: ICoreOutput) => {
      if (data.ok) {
        ...
        naviage(
          `/search-question?pagesize=${pagesize}&page=1&normid=${query.normid}`
        );
      }
    },
  });
  ...
  const {
    ...
    getValues,
    reset,
  } = useForm<ICreateQuestionFromInput>({
    mode: "onChange",
  });
  ...
  const onSubmit = () => {
    if (
      ...
      typeof searchQuestionData?.totalResult === "number" &&
      query.normid
    ) {
      const { file } = getValues();
      createQuestionMutation.mutate({
        page: searchQuestionData?.totalResult + 1,
        normId: +query.normid,
        file: file[0],
      });
    }
    ...
    <div className="mt-5 max-w-5xl mx-auto rounded-2xl  flex flex-col justify-center items-center p-10 font-bold">
      ...
      <div className="grid grid-cols-1 grid-rows-none  w-full  mx-auto grid-flow-col gap-20 ">
        <div className=" row-start-1 ">
          <h4 className="font-semibold text-2xl  text-center ">문제 등록</h4>
          <div className="flex justify-between items-center min-w-max">
            <div>
              <Link
                className="hover:underline "
                to={`/search-question?pagesize=${pagesize}&page=1&normid=${query.normid}`}
              >
                문제 목록 <FontAwesomeIcon icon={faChevronRight} />
              </Link>
              <span> 등록</span>{" "}
              <span className="col-start-1 col-row-1 my-2">
                ({" "}
                {typeof searchQuestionData?.totalResult === "number" &&
                  searchQuestionData?.totalResult + 1}
                페이지 )
              </span>
            </div>
          </div>
